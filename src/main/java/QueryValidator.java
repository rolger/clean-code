...
public class QueryValidator {
	...
	/**
	 * Validate the path, and value type, returning the mapped field for the field
	 * at the path
	 */
	static MappedField validateQuery(final Class clazz, final Mapper mapper, final StringBuilder origProp,
			final FilterOperator op, final Object val, final boolean validateNames, final boolean validateTypes) {
		...
		if (!origProp.substring(0, 1).equals("$")) {
			...
			for (int i = 0;;) {
				...
				if (!fieldIsArrayOperator) {
					...
					if (validateNames && !canQueryPast(mf)) {
						throw new ValidationException(String.format(
								"Cannot use dot-notation past '%s' in '%s'; found while" + " validating - %s", part,
								mc.getClazz().getName(), prop));
					}

					if (mf == null && clazz.isInterface()) {
						break;
					} else if (mf == null) {
						throw new ValidationException(String.format("The field '%s' could not be found in '%s'", prop,
								mc.getClazz().getName()));
					}
			...
			if (validateTypes && mf != null) {
				...
				boolean compatibleForType = isCompatibleForOperator(mc, mf, mf.getType(), op, val,
						typeValidationFailures, mapper);
				...
				boolean compatibleForSubclass = isCompatibleForOperator(mc, mf, mf.getSubClass(), op, val,
						subclassValidationFailures, mapper);
				...
				if ((mf.isSingleValue() && !compatibleForType)
					...
					if (LOG.isWarningEnabled()) {
						LOG.warning(String.format(
								...
								val.getClass().getName(), mf.getDeclaringClass().getName(), mf.getJavaFieldName(),
								mf.getType().getName()));
	...
	static boolean isCompatibleForOperator(final MappedClass mappedClass, final MappedField mappedField,
			final Class<?> type, final FilterOperator op, final Object value,
			final List<ValidationFailure> validationFailures, Mapper mapper) {
		// TODO: it's really OK to have null values? I think this is to prevent null
		// pointers further down,
		// but I want to move the null check into the operations that care whether they
		// allow nulls or not.
		if (value == null || type == null) {
			return true;
		}

		boolean validationApplied = ExistsOperationValidator.getInstance().apply(mappedField, op, value,
				validationFailures)
				...
				|| EntityTypeAndIdValueValidator.getInstance().apply(mapper, type, value, validationFailures)
